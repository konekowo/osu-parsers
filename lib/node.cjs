...
let lzmaJsSimpleV2 = require('lzma-js-simple-v2');
...
class BeatmapColorDecoder {
  static handleLine(line, output) {
    ...
    const color = new osuClasses.Color4(rgba[0], rgba[1], rgba[2], rgba[3]);
...
class StoryboardEventDecoder {
  ...
  static _handleElement(line, storyboard) {
    ...
    if ((_a = this._storyboardSprite) === null || _a === void 0 ? void 0 : _a.hasCommands) {
    ...
    switch (eventType) {
      case osuClasses.EventType.Video: {
        const layer = storyboard.getLayerByType(osuClasses.LayerType.Video);
        const offset = Parsing.parseInt(data[1]);
        const path = data[2].replace(/"/g, '');

        layer.elements.push(new osuClasses.StoryboardVideo(path, offset));

        return;
      }
      case osuClasses.EventType.Sprite: {
        ...
        this._storyboardSprite = new osuClasses.StoryboardSprite(path, origin, anchor, new osuClasses.Vector2(x, y));
        layer.elements.push(this._storyboardSprite);

        return;
      }
      case osuClasses.EventType.Animation: {
        const layer = storyboard.getLayerByType(this.parseLayerType(data[1]));
        const origin = this.parseOrigin(data[2]);
        const anchor = this.convertOrigin(origin);
        const path = data[3].replace(/"/g, '');
        ...
        this._storyboardSprite = new osuClasses.StoryboardAnimation(path, origin, anchor, new osuClasses.Vector2(x, y), frameCount, frameDelay, loopType);
        layer.elements.push(this._storyboardSprite);

        return;
      }
      case osuClasses.EventType.Sample: {
        ...
        const sample = new osuClasses.StoryboardSample(path, time, volume);
  ...
  static _handleCompoundOrCommand(line) {
    ...
    switch (compoundType) {
      case osuClasses.CompoundType.Trigger: {
        this._timelineGroup = (_a = this._storyboardSprite) === null || _a === void 0 ? void 0 : _a.addTrigger(data[1], data.length > 2 ? Parsing.parseFloat(data[2]) : -Infinity, data.length > 3 ? Parsing.parseFloat(data[3]) : Infinity, data.length > 4 ? Parsing.parseInt(data[4]) : 0);

        return;
      }
      case osuClasses.CompoundType.Loop: {
        this._timelineGroup = (_b = this._storyboardSprite) === null || _b === void 0 ? void 0 : _b.addLoop(Parsing.parseFloat(data[1]), Math.max(0, Parsing.parseInt(data[2]) - 1));
  ...
  static _handleCommand(line) {
    ...
    switch (type) {
      case osuClasses.CommandType.Fade: {
        const startValue = Parsing.parseFloat(data[4]);
        const endValue = data.length > 5 ? Parsing.parseFloat(data[5]) : startValue;

        (_a = this._timelineGroup) === null || _a === void 0 ? void 0 : _a.alpha.add(type, easing, startTime, endTime, startValue, endValue);

        return;
      }
      case osuClasses.CommandType.Scale: {
        const startValue = Parsing.parseFloat(data[4]);
        const endValue = data.length > 5 ? Parsing.parseFloat(data[5]) : startValue;

        (_b = this._timelineGroup) === null || _b === void 0 ? void 0 : _b.scale.add(type, easing, startTime, endTime, startValue, endValue);

        return;
      }
      case osuClasses.CommandType.VectorScale: {
        ...
        (_c = this._timelineGroup) === null || _c === void 0 ? void 0 : _c.vectorScale.add(type, easing, startTime, endTime, new osuClasses.Vector2(startX, startY), new osuClasses.Vector2(endX, endY));

        return;
      }
      case osuClasses.CommandType.Rotation: {
        const startValue = Parsing.parseFloat(data[4]);
        const endValue = data.length > 5 ? Parsing.parseFloat(data[5]) : startValue;

        (_d = this._timelineGroup) === null || _d === void 0 ? void 0 : _d.rotation.add(type, easing, startTime, endTime, startValue, endValue);

        return;
      }
      case osuClasses.CommandType.Movement: {
        const startX = Parsing.parseFloat(data[4]);
        const startY = Parsing.parseFloat(data[5]);
        const endX = data.length > 6 ? Parsing.parseFloat(data[6]) : startX;
        const endY = data.length > 7 ? Parsing.parseFloat(data[7]) : startY;

        (_e = this._timelineGroup) === null || _e === void 0 ? void 0 : _e.x.add(osuClasses.CommandType.MovementX, easing, startTime, endTime, startX, endX);
        (_f = this._timelineGroup) === null || _f === void 0 ? void 0 : _f.y.add(osuClasses.CommandType.MovementY, easing, startTime, endTime, startY, endY);

        return;
      }
      case osuClasses.CommandType.MovementX: {
        const startValue = Parsing.parseFloat(data[4]);
        const endValue = data.length > 5 ? Parsing.parseFloat(data[5]) : startValue;

        (_g = this._timelineGroup) === null || _g === void 0 ? void 0 : _g.x.add(type, easing, startTime, endTime, startValue, endValue);

        return;
      }
      case osuClasses.CommandType.MovementY: {
        const startValue = Parsing.parseFloat(data[4]);
        const endValue = data.length > 5 ? Parsing.parseFloat(data[5]) : startValue;

        (_h = this._timelineGroup) === null || _h === void 0 ? void 0 : _h.y.add(type, easing, startTime, endTime, startValue, endValue);

        return;
      }
      case osuClasses.CommandType.Color: {
        ...
        (_j = this._timelineGroup) === null || _j === void 0 ? void 0 : _j.color.add(type, easing, startTime, endTime, new osuClasses.Color4(startRed, startGreen, startBlue, 1), new osuClasses.Color4(endRed, endGreen, endBlue, 1));

        return;
      }
      case osuClasses.CommandType.Parameter: {
  ...
  static _handleParameterCommand(data) {
    let _a, _b, _c;
    const type = osuClasses.CommandType.Parameter;
    const easing = Parsing.parseInt(data[1]);
    const startTime = Parsing.parseInt(data[2]);
    const endTime = Parsing.parseInt(data[3] || data[2]);
    ...
    switch (parameter) {
      case osuClasses.ParameterType.BlendingMode: {
        const startValue = osuClasses.BlendingParameters.Additive;
        const endValue = startTime === endTime
          ? osuClasses.BlendingParameters.Additive
          : osuClasses.BlendingParameters.Inherit;

        (_a = this._timelineGroup) === null || _a === void 0 ? void 0 : _a.blendingParameters.add(type, easing, startTime, endTime, startValue, endValue, parameter);

        return;
      }
      case osuClasses.ParameterType.HorizontalFlip:
        (_b = this._timelineGroup) === null || _b === void 0 ? void 0 : _b.flipH.add(type, easing, startTime, endTime, true, startTime === endTime, parameter);

        return;
      case osuClasses.ParameterType.VerticalFlip: {
        (_c = this._timelineGroup) === null || _c === void 0 ? void 0 : _c.flipV.add(type, easing, startTime, endTime, true, startTime === endTime, parameter);
  ...
  static parseEventType(input) {
    if (input.startsWith(' ') || input.startsWith('_')) {
      return osuClasses.EventType.StoryboardCommand;
    }

    try {
      return Parsing.parseEnum(osuClasses.EventType, input);
    }
    catch {
  ...
  static parseLayerType(input) {
    try {
      return Parsing.parseEnum(osuClasses.LayerType, input);
    }
    catch {
  ...
  static parseOrigin(input) {
    try {
      return Parsing.parseEnum(osuClasses.Origins, input);
    }
    catch {
      return osuClasses.Origins.TopLeft;
    }
  }
  static convertOrigin(origin) {
    switch (origin) {
      case osuClasses.Origins.TopLeft: return osuClasses.Anchor.TopLeft;
      case osuClasses.Origins.TopCentre: return osuClasses.Anchor.TopCentre;
      case osuClasses.Origins.TopRight: return osuClasses.Anchor.TopRight;
      case osuClasses.Origins.CentreLeft: return osuClasses.Anchor.CentreLeft;
      case osuClasses.Origins.Centre: return osuClasses.Anchor.Centre;
      case osuClasses.Origins.CentreRight: return osuClasses.Anchor.CentreRight;
      case osuClasses.Origins.BottomLeft: return osuClasses.Anchor.BottomLeft;
      case osuClasses.Origins.BottomCentre: return osuClasses.Anchor.BottomCentre;
      case osuClasses.Origins.BottomRight: return osuClasses.Anchor.BottomRight;
    }

    return osuClasses.Anchor.TopLeft;
  }
  static parseLoopType(input) {
    try {
      return Parsing.parseEnum(osuClasses.LoopType, input);
    }
    catch {
      return osuClasses.LoopType.LoopForever;
    }
  }
...
class BeatmapEventDecoder {
  static handleLine(line, beatmap, sbLines, offset) {
    ...
    switch (eventType) {
      ...
      case osuClasses.EventType.Sprite:
      case osuClasses.EventType.Animation:
      case osuClasses.EventType.StoryboardCommand:
        if (sbLines) {
          sbLines.push(line);
        }
    }
  ...
  static handleLine(line, beatmap, offset) {
    ...
    switch (key.trim()) {
      ...
      case 'CountdownOffset':
        ...
        break;
      case 'SampleSet':
        beatmap.general.sampleSet = osuClasses.SampleSet[value];
        break;
...
class HittableObject extends osuClasses.HitObject {
  constructor() {
    super(...arguments);
...
class HoldableObject extends osuClasses.HitObject {
  constructor() {
    super(...arguments);
    this.endTime = 0;
    this.nodeSamples = [];
...
class SlidableObject extends osuClasses.HitObject {
  constructor() {
    super(...arguments);
    this.repeats = 0;
    this.velocity = 1;
...
class SpinnableObject extends osuClasses.HitObject {
  constructor() {
    super(...arguments);
    this.endTime = 0;
    this.isNewCombo = false;
...
class StoryboardEventEncoder {
  ...
  static encodeVideos(storyboard) {
    const encoded = [];
    const video = storyboard.getLayerByType(osuClasses.LayerType.Video);
  ...
  static encodeStoryboard(storyboard) {
    ...
    const background = storyboard.getLayerByType(osuClasses.LayerType.Background);
    ...
    const fail = storyboard.getLayerByType(osuClasses.LayerType.Fail);
    ...
    const pass = storyboard.getLayerByType(osuClasses.LayerType.Pass);
    ...
    const foreground = storyboard.getLayerByType(osuClasses.LayerType.Foreground);
    ...
    const overlay = storyboard.getLayerByType(osuClasses.LayerType.Overlay);
  ...
  static encodeStoryboardLayer(layer) {
    ...
    layer.elements.forEach((element) => {
      ...
      (_a = elementWithCommands === null || elementWithCommands === void 0 ? void 0 : elementWithCommands.loops) === null || _a === void 0 ? void 0 : _a.forEach((loop) => {
      ...
      if (((_b = elementWithCommands === null || elementWithCommands === void 0 ? void 0 : elementWithCommands.timelineGroup) === null || _b === void 0 ? void 0 : _b.commands.length) > 0) {
        encoded.push(this.encodeTimelineGroup(elementWithCommands.timelineGroup));
      }

      (_c = elementWithCommands === null || elementWithCommands === void 0 ? void 0 : elementWithCommands.triggers) === null || _c === void 0 ? void 0 : _c.forEach((trigger) => {
  ...
  static encodeStoryboardElement(element, layer) {
    if (element instanceof osuClasses.StoryboardAnimation) {
      return [
        'Animation',
        layer,
        osuClasses.Origins[element.origin],
        `"${element.filePath}"`,
    ...
    if (element instanceof osuClasses.StoryboardSprite) {
      return [
        'Sprite',
        layer,
        osuClasses.Origins[element.origin],
    ...
    if (element instanceof osuClasses.StoryboardSample) {
      return [
        'Sample',
        element.startTime,
        layer,
    ...
    if (element instanceof osuClasses.StoryboardVideo) {
      return [
        'Video',
        element.startTime,
        element.filePath,
  ...
  static encodeCompound(compound, depth = 1) {
    ...
    if (compound instanceof osuClasses.CommandLoop) {
      return indentation + [
        compound.type,
    ...
    if (compound instanceof osuClasses.CommandTrigger) {
      return indentation + [
        compound.type,
  ...
  static encodeTimelineGroup(timelineGroup, depth = 1) {
    ...
    for (let i = 0; i < commands.length; ++i) {
      ...
      if (i < commands.length - 1) {
        ...
        const currentMoveX = current.type === osuClasses.CommandType.MovementX;
        const nextMoveY = next.type === osuClasses.CommandType.MovementY;
  ...
  static encodeMoveCommand(moveX, moveY) {
    const encoded = [
      osuClasses.CommandType.Movement,
      moveX.easing,
      moveX.startTime,
  ...
  static _encodeCommandParams(command) {
    if (command.type === osuClasses.CommandType.Parameter) {
      return command.parameter;
    }

    if (command.type === osuClasses.CommandType.Color) {
  ...
  static _areValuesEqual(command) {
    if (command.type === osuClasses.CommandType.VectorScale) {
  ...
  static encodeEventSection(beatmap) {
    ...
    if (events.breaks && events.breaks.length > 0) {
      events.breaks.forEach((b) => {
        encoded.push(`${osuClasses.EventType[osuClasses.EventType.Break]},${b.startTime},${b.endTime}`);
      });
    }
...
class BeatmapGeneralEncoder {
  static encodeGeneralSection(beatmap) {
    ...
    encoded.push(`SampleSet:${osuClasses.SampleSet[general.sampleSet]}`);
...
class BeatmapHitObjectEncoder {
  static encodeHitObjects(beatmap) {
    ...
    hitObjects.forEach((hitObject) => {
      ...
      const startPosition = new osuClasses.Vector2(position ? position.x : 256, position ? position.y : 192);
      ...
      else if (hitObject.hitType & osuClasses.HitType.Spinner) {
        const spinner = hitObject;

        extras.push(this.encodeEndTimeData(spinner));
      }
      else if (hitObject.hitType & osuClasses.HitType.Hold) {
      ...
      const normal = hitObject.samples.find((s) => {
        return s.hitSound === osuClasses.HitSound[osuClasses.HitSound.Normal];
      });
      const addition = hitObject.samples.find((s) => {
        return s.hitSound !== osuClasses.HitSound[osuClasses.HitSound.Normal];
      });
      let normalSet = osuClasses.SampleSet.None;
      let additionSet = osuClasses.SampleSet.None;

      if (normal) {
        normalSet = osuClasses.SampleSet[normal.sampleSet];
      }

      if (addition) {
        additionSet = osuClasses.SampleSet[addition.sampleSet];
      }
      ...
      const extrasLine = hitObject.hitType & osuClasses.HitType.Hold
        ? extras.join(':')
        : extras.join(',');
  ...
  static encodePathData(slider, offset) {
    ...
    slider.path.controlPoints.forEach((point, i) => {
      if (point.type !== null) {
        let needsExplicitSegment = point.type !== lastType
                    || point.type === osuClasses.PathType.PerfectCurve;
    ...
    slider.nodeSamples.forEach((node, nodeIndex) => {
      adds[nodeIndex] = osuClasses.HitSound.None;
      sets[nodeIndex] = [osuClasses.SampleSet.None, osuClasses.SampleSet.None];
      node.forEach((sample, sampleIndex) => {
        if (sampleIndex === 0) {
          sets[nodeIndex][0] = osuClasses.SampleSet[sample.sampleSet];
        }
        else {
          adds[nodeIndex] |= osuClasses.HitSound[sample.hitSound];
          sets[nodeIndex][1] = osuClasses.SampleSet[sample.sampleSet];
        }
      });
    });
...
class BeatmapTimingPointEncoder {
  ...
  static encodeGroup(group, useTiming = false) {
    ...
    let sampleSet = osuClasses.SampleSet.None;
    let customIndex = 0;
    let volume = 100;

    if (samplePoint !== null) {
      sampleSet = osuClasses.SampleSet[samplePoint.sampleSet];
      customIndex = samplePoint.customIndex;
      volume = samplePoint.volume;
    }

    let effects = osuClasses.EffectType.None;

    if (effectPoint !== null) {
      const kiai = effectPoint.kiai
        ? osuClasses.EffectType.Kiai
        : osuClasses.EffectType.None;
      const omitFirstBarLine = effectPoint.omitFirstBarLine
        ? osuClasses.EffectType.OmitFirstBarLine
        : osuClasses.EffectType.None;

      effects |= kiai | omitFirstBarLine;
    }

    let timeSignature = osuClasses.TimeSignature.SimpleQuadruple;
    let uninherited = 0;
  ...
  static updateActualPoints(group) {
    ...
    group.controlPoints.forEach((point) => {
      if (point.pointType === osuClasses.ControlPointType.DifficultyPoint
                && !point.isRedundant(this.lastDifficultyPoint)) {
        this.lastDifficultyPoint = point;
      }

      if (point.pointType === osuClasses.ControlPointType.EffectPoint
                && !point.isRedundant(this.lastEffectPoint)) {
        this.lastEffectPoint = point;
      }

      if (point.pointType === osuClasses.ControlPointType.SamplePoint
                && !point.isRedundant(this.lastSamplePoint)) {
        this.lastSamplePoint = point;
      }

      if (point.pointType === osuClasses.ControlPointType.TimingPoint) {
        timingPoint = point;
      }
    });
...
class ReplayEncoder {
  ...
  static encodeReplayFrames(frames, beatmap) {
    ...
    if (frames) {
      ...
      frames.forEach((frame) => {
        ...
        const encodedData = [
          time - lastTime,
          (_a = legacyFrame === null || legacyFrame === void 0 ? void 0 : legacyFrame.mouseX) !== null && _a !== void 0 ? _a : 0,
          (_b = legacyFrame === null || legacyFrame === void 0 ? void 0 : legacyFrame.mouseY) !== null && _b !== void 0 ? _b : 0,
          (_c = legacyFrame === null || legacyFrame === void 0 ? void 0 : legacyFrame.buttonState) !== null && _c !== void 0 ? _c : osuClasses.ReplayButtonState.None,
        ];
  ...
  static _getLegacyFrame(frame, beatmap) {
    if (frame instanceof osuClasses.LegacyReplayFrame) {
      return frame;
    }
...
class BeatmapEncoder {
  async encodeToPath(path$1, beatmap) {
    if (!path$1.endsWith(FileFormat.Beatmap)) {
      path$1 += FileFormat.Beatmap;
    }

    try {
      await promises.mkdir(path.dirname(path$1), { recursive: true });
      await promises.writeFile(path$1, await this.encodeToString(beatmap));
    }
    catch (err) {
  ...
  encodeToString(beatmap) {
    ...
    const fileFormat = (_a = beatmap.fileFormat) !== null && _a !== void 0 ? _a : BeatmapEncoder.FIRST_LAZER_VERSION;
    const encoded = [
...
class StoryboardEncoder {
  async encodeToPath(path$1, storyboard) {
    if (!path$1.endsWith(FileFormat.Storyboard)) {
      path$1 += FileFormat.Storyboard;
    }

    try {
      await promises.mkdir(path.dirname(path$1), { recursive: true });
      await promises.writeFile(path$1, await this.encodeToString(storyboard));
    }
    catch (err) {
      const reason = err.message || err;
  ...
  encodeToString(storyboard) {
    if (!(storyboard instanceof osuClasses.Storyboard)) {
      return '';
    }
  ...
  return {
    decompress(data) {
      return new Promise((res, rej) => {
        lzmaJsSimpleV2.decompress(data, (result, err) => {
          err ? rej(err) : res(typeof result === 'string'
            ? result
    ...
    compress(data) {
      return new Promise((res, rej) => {
        lzmaJsSimpleV2.compress(data, 1, (result, err) => {
...
class ScoreEncoder {
  async encodeToPath(path$1, score, beatmap) {
    if (!path$1.endsWith(FileFormat.Replay)) {
      path$1 += FileFormat.Replay;
    }
    ...
    try {
      await promises.mkdir(path.dirname(path$1), { recursive: true });
      await promises.writeFile(path$1, new Uint8Array(data));
    }
    catch (err) {
      const reason = err.message || err;
  ...
  async encodeToBuffer(score, beatmap) {
    ...
    try {
      ...
      writer.writeInteger((_c = (_b = score.replay) === null || _b === void 0 ? void 0 : _b.gameVersion) !== null && _c !== void 0 ? _c : ScoreEncoder.DEFAULT_GAME_VERSION);
      writer.writeString((_d = score.info.beatmapHashMD5) !== null && _d !== void 0 ? _d : '');
      writer.writeString(score.info.username);
      writer.writeString((_f = (_e = score.replay) === null || _e === void 0 ? void 0 : _e.hashMD5) !== null && _f !== void 0 ? _f : '');
      ...
      writer.writeInteger(((_h = (_g = score.info.mods) === null || _g === void 0 ? void 0 : _g.bitwise) !== null && _h !== void 0 ? _h : Number(score.info.rawMods)) || 0);
      writer.writeString(ReplayEncoder.encodeLifeBar((_k = (_j = score.replay) === null || _j === void 0 ? void 0 : _j.lifeBar) !== null && _k !== void 0 ? _k : []));
...
class BeatmapHitObjectDecoder {
  ...
  static addComboOffset(hitObject, beatmap) {
    ...
    const comboOffset = Math.trunc((hitObject.hitType & osuClasses.HitType.ComboOffset) >> 4);
    const newCombo = !!(hitObject.hitType & osuClasses.HitType.NewCombo);

    if ((hitObject.hitType & osuClasses.HitType.Normal) || (hitObject.hitType & osuClasses.HitType.Slider)) {
  ...
  static addExtras(data, hitObject, bankInfo, offset, fileFormat) {
    if ((hitObject.hitType & osuClasses.HitType.Normal) && data.length > 0) {
    ...
    if (hitObject.hitType & osuClasses.HitType.Hold) {
  ...
  static getSliderNodeSamples(extras, slider, bankInfo) {
    ...
    if (extras.length > 3 && extras[3].length > 0) {
      ...
      for (let i = 0; i < nodes; ++i) {
        ...
        nodeSoundTypes[i] = parseInt(adds[i]) || osuClasses.HitSound.None;
      }
    }
  ...
  static *convertPoints(points, endPoint, isFirst, offset, fileFormat) {
    ...
    if (readOffset === 1) {
      vertices[0] = new osuClasses.PathPoint();
    }
    ...
    if (type === osuClasses.PathType.PerfectCurve) {
      if (vertices.length !== 3) {
        type = osuClasses.PathType.Bezier;
      }
      else if (isLinear(vertices)) {
        type = osuClasses.PathType.Linear;
      }
    }
    ...
    while (++endIndex < vertices.length - endPointLength) {
      ...
      if (type === osuClasses.PathType.Catmull && endIndex > 1 && isStableBeatmap) {
        continue;
      }
    ...
    function readPoint(point, offset) {
      ...
      const pos = new osuClasses.Vector2(coords[0], coords[1]).fsubtract(offset);

      return new osuClasses.PathPoint(pos);
    }
  ...
  static convertPathType(type) {
    switch (type) {
      default:
      case 'C':
        return osuClasses.PathType.Catmull;
      case 'B':
        return osuClasses.PathType.Bezier;
      case 'L':
        return osuClasses.PathType.Linear;
      case 'P':
        return osuClasses.PathType.PerfectCurve;
    }
  }
  static readCustomSampleBanks(hitSample, bankInfo) {
    ...
    if (bankInfo.additionSet === osuClasses.SampleSet.None) {
  ...
  static convertSoundType(type, bankInfo) {
    if (bankInfo.filename) {
      const sample = new osuClasses.HitSample();
    ...
    const soundTypes = [new osuClasses.HitSample()];

    soundTypes[0].hitSound = osuClasses.HitSound[osuClasses.HitSound.Normal];
    soundTypes[0].sampleSet = osuClasses.SampleSet[bankInfo.normalSet];
    soundTypes[0].isLayered = type !== osuClasses.HitSound.None && !(type & osuClasses.HitSound.Normal);

    if (type & osuClasses.HitSound.Finish) {
      const sample = new osuClasses.HitSample();
    ...
    if (type & osuClasses.HitSound.Whistle) {
      const sample = new osuClasses.HitSample();

      sample.hitSound = osuClasses.HitSound[osuClasses.HitSound.Whistle];
      soundTypes.push(sample);
    }

    if (type & osuClasses.HitSound.Clap) {
      const sample = new osuClasses.HitSample();
    ...
    soundTypes.forEach((sound, i) => {
      sound.sampleSet = i !== 0
        ? osuClasses.SampleSet[bankInfo.additionSet]
        : osuClasses.SampleSet[bankInfo.normalSet];
  ...
  static createHitObject(hitType) {
    if (hitType & osuClasses.HitType.Normal) {
      return new HittableObject();
    }

    if (hitType & osuClasses.HitType.Slider) {
      return new SlidableObject();
    }

    if (hitType & osuClasses.HitType.Spinner) {
      return new SpinnableObject();
    }

    if (hitType & osuClasses.HitType.Hold) {
...
class BeatmapTimingPointDecoder {
  static handleLine(line, beatmap, offset) {
    ...
    const data = line.split(',');
    let timeSignature = osuClasses.TimeSignature.SimpleQuadruple;
    let sampleSet = osuClasses.SampleSet[osuClasses.SampleSet.None];
    let customIndex = 0;
    let volume = 100;
    ...
    if (data.length > 2) {
      switch (data.length) {
        ...
        case 4: sampleSet = osuClasses.SampleSet[Parsing.parseInt(data[3])];
    ...
    if (timingChange) {
      const timingPoint = new osuClasses.TimingPoint();
    ...
    const difficultyPoint = new osuClasses.DifficultyPoint();
    ...
    const effectPoint = new osuClasses.EffectPoint();

    effectPoint.kiai = (effects & osuClasses.EffectType.Kiai) > 0;
    effectPoint.omitFirstBarLine = (effects & osuClasses.EffectType.OmitFirstBarLine) > 0;
    ...
    const samplePoint = new osuClasses.SamplePoint();
...
class ReplayDecoder {
  ...
  static handleLifeBarFrame(frameData) {
    return new osuClasses.LifeBarFrame(Parsing.parseInt(frameData[0]), Parsing.parseFloat(frameData[1]));
  }
  ...
  static handleReplayFrame(frameData) {
    return new osuClasses.LegacyReplayFrame(0.0, Parsing.parseFloat(frameData[0]), new osuClasses.Vector2(Parsing.parseFloat(frameData[1], Parsing.MAX_COORDINATE_VALUE), Parsing.parseFloat(frameData[2], Parsing.MAX_COORDINATE_VALUE)), Parsing.parseInt(frameData[3]));
  }
}
...
class SerializationReader {
  ...
  readBytes(length) {
    const bytes = this.view.buffer.slice(this._bytesRead, this._bytesRead + length);
...
class Decoder {
  ...
  async _getFileUpdateDate(path) {
    try {
      return (await promises.stat(path)).mtime;
    }
    catch {
...
class SectionDecoder extends Decoder {
  ...
  _parseSectionData(line, output) {
    ...
    if (!(outputWithColors === null || outputWithColors === void 0 ? void 0 : outputWithColors.colors)) {
      return;
    }
  ...
  _setEnabledSections(options) {
    this._sectionMap.set(Section.Colours, options === null || options === void 0 ? void 0 : options.parseColours);
  }
}
...
class StoryboardDecoder extends SectionDecoder {
  ...
  decodeFromLines(firstData, secondData) {
    const storyboard = new osuClasses.Storyboard();
...
class BeatmapDecoder extends SectionDecoder {
  ...
  decodeFromLines(data, options) {
    const beatmap = new osuClasses.Beatmap();
  ...
  _setEnabledSections(options) {
    ...
    this._sectionMap.set(Section.General, options === null || options === void 0 ? void 0 : options.parseGeneral);
    this._sectionMap.set(Section.Editor, options === null || options === void 0 ? void 0 : options.parseEditor);
    this._sectionMap.set(Section.Metadata, options === null || options === void 0 ? void 0 : options.parseMetadata);
    this._sectionMap.set(Section.Difficulty, options === null || options === void 0 ? void 0 : options.parseDifficulty);
    this._sectionMap.set(Section.Events, options === null || options === void 0 ? void 0 : options.parseEvents);
    this._sectionMap.set(Section.TimingPoints, options === null || options === void 0 ? void 0 : options.parseTimingPoints);
    this._sectionMap.set(Section.HitObjects, options === null || options === void 0 ? void 0 : options.parseHitObjects);
  }
  _shouldParseStoryboard(options) {
    let _a, _b;
    const parsingOptions = options;
    const storyboardFlag = (_a = parsingOptions === null || parsingOptions === void 0 ? void 0 : parsingOptions.parseStoryboard) !== null && _a !== void 0 ? _a : options;
    const parseSb = typeof storyboardFlag === 'boolean' ? storyboardFlag : true;
    const parseEvents = (_b = parsingOptions === null || parsingOptions === void 0 ? void 0 : parsingOptions.parseEvents) !== null && _b !== void 0 ? _b : true;
...
class ScoreDecoder extends Decoder {
  ...
  async decodeFromBuffer(buffer, parseReplay = true) {
    ...
    try {
      ...
      return new osuClasses.Score(scoreInfo, replay);
    }
    catch {
      return new osuClasses.Score(scoreInfo, replay);
    }
...
exports.HittableObject = HittableObject;
exports.HoldableObject = HoldableObject;
exports.ScoreDecoder = ScoreDecoder;
exports.ScoreEncoder = ScoreEncoder;
exports.SlidableObject = SlidableObject;
exports.SpinnableObject = SpinnableObject;
